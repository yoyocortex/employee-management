...
export default class Employee extends Component {
    ...
    componentDidMount = async () => {
        ...
        this.setState({ employeeList: getEmployeeLists });
    };
    ...
    add = async (formData) => {
        let getEmployeeLists = await onSubmitNewEmployee(formData);
        this.setState({ employeeList: getEmployeeLists });
    }
    
    update = async (id, formData) => {
        let getEmployeeLists = await updateEmployee(id, formData);
        this.setState({ employeeList: getEmployeeLists });
    }
    
    delete = async (id) => {
        let getEmployeeLists = await deleteEmployee(id);
        this.setState({ employeeList: getEmployeeLists });
    }

    render() {
        const { style } = this.props;
        const { open, employeeList, list } = this.state;
